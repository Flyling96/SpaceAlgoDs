...
#pragma warning restore 109
        ...
        public Mesh RefreshUpIntersect(ProBuilderMesh other)
        {
            if (other == null)
            {
                return null;
            ...
            //var selfModel = new Csg.Model(gameObject);
            //var polygons = selfModel.ToPolygons();
            //for (int i = polygons.Count - 1; i > -1; i--)
            //{
            //    var polygon = polygons[i];
            //    var normal = polygon.plane.normal.normalized;
            //    if (normal.y >= 0.8f)
            //    {
            //        polygons.RemoveAt(i);
            //    }
            //}
            //selfModel = new Csg.Model(polygons);

            var model = Csg.Boolean.IntersectOne(gameObject, other.gameObject);
            if(model != null)
            {
                var polygons = model.ToPolygons();
                for (int i = polygons.Count - 1; i > -1; i--)
                {
                    var polygon = polygons[i];
                    var normal = polygon.plane.normal.normalized;
                ...
                if(polygons.Count > 0)
                {
                    var planeModel = new Csg.SimpleModel(polygons);
                    return (Mesh)planeModel;
                }
            }
